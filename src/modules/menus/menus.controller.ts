...
export class MenusController {
   ...
   */
  @Post()
  @ApiOperation({ summary: 'Create a new menu', description: 'Creates a new menu with specified details for the authenticated company.' })
   ...
   */
  @Get()
  @ApiOperation({ summary: 'List all menus for the company', description: 'Retrieves a list of all menus associated with the authenticated company.' })
   ...
   */
  @Get('published')
  @ApiOperation({ summary: 'List all currently published and active menus for users', description: 'Fetches menus that are currently published and within their active date range for the authenticated company.' })
   ...
   */
  @Get(':id')
  @ApiOperation({ summary: 'Get a single menu by ID', description: 'Retrieves a specific menu by its unique ID for the authenticated company, including its full structure of days, options, and shifts.' })
   ...
   */
  @Patch(':id')
  @ApiOperation({ summary: 'Update a menu', description: 'Updates an existing menu identified by its ID for the authenticated company.' })
  @ApiParam({ name: 'id', description: 'Unique identifier of the menu to update' })
   ...
   */
  @Delete(':id')
  @ApiOperation({ summary: 'Delete a menu', description: 'Deletes a menu identified by its ID for the authenticated company.' })
  @ApiParam({ name: 'id', description: 'Unique identifier of the menu to delete' })
   ...
   */
  @Post('options')
  @ApiOperation({ summary: 'Add a menu option to a menu day', description: 'Adds a new menu option (dish) to a specific day within an active menu. This links a product to shifts on a particular date.' })
  @ApiResponse({ status: 201, description: 'The menu option has been successfully added.' })
  @ApiResponse({ status: 400, description: 'Bad Request. Invalid input data or invalid shifts.' })
  @ApiResponse({ status: 401, description: 'Unauthorized.' })
  @ApiResponse({ status: 403, description: 'Forbidden. Tenant access denied.' })
  @ApiResponse({ status: 404, description: 'Active menu for the date not found for this company.' })
